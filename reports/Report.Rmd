...
```
**Enrollments regarding Age range per Cycle**   
In the below graph it is visualized the Age range per cycle. This display the trend per age of enrollments. 

```{r}
ga = ggplot(dfdatacycle_4_5, aes(x=age, y=n, group=cycles)) +
  geom_line(aes(color=cycles))+
  geom_point(aes(color=cycles)) +
  labs(title = "Enrollments regarding Age /Cycles",x = "Age-Range", y = "Number of Ocurances")
print(ga)
```
